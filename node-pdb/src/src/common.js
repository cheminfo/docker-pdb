...
module.exports = {
  ...
  processPdbAssemblies: function (files) {
    return new Promise(function (resolve, reject) {
      if (files && files.length > 0) {
        async.mapSeries(files, module.exports.processPdbAssembly, function (
          err
        ) {
          if (err) return reject(err);
          return resolve();
  ...
  return new Promise(function (resolve, reject) {
    fs.readFile(filename, function (err, data) {
      ...
      zlib.gunzip(data, function (err, buffer) {
        ...
        if (err) return reject(err);
        pymol(pdbEntry._id, buffer, config.pymol)
          .then(
            function (buff) {
              ...
              for (var i = 0; i < buff.length; i++) {
                pdbEntry._attachments[
                  `${
                    config.pymol[i].width
                  }x${
                    config.pymol[i].height
                  }.png`
                ] = {
              ...
              if (buffer.length < MAX_BUFFER_LENGTH) {
                ...
                console.log(
                  `Not adding ${
                    pdbEntry._id
                  }.pdb1 to database (file is too big)`
                );
              }
            ...
            function (err) {
              console.error(
                `An error occured while generating the image with pymol${
                  pdbEntry._id}`,
                err
              );
