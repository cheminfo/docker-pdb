/**
 * Channel-based renderer for the scripting API. Each Selection has
...
export interface Channels {
  ...
  /**
   * Toggle visibility of every component the channel system created for the
   * given kind across every selection — used by the `ms.<kind>` aggregators
   * to temporarily hide all atoms / bonds / ribbon / surface / label
   * representations without dropping their state.
   */
  setKindVisibility: (kind: ChannelKey, visible: boolean) => void;
  resetState: () => void;
}
...
export function createChannels(context: ChannelContext): Channels {
  ...
  async function applyLabel(
    ...
    const typeParams: Record<string, unknown> = {
      ...
      // Render labels through occluding atoms so axis-tip names stay
      // legible regardless of camera angle. `xrayShaded: 'on'` is a
      // `BaseGeometry` flag that disables full depth occlusion. The
      // border adds a black outline so the text reads on any background.
      xrayShaded: 'on',
      borderWidth: 0.25,
  ...
  return {
    ...
    setKindVisibility(kind, visible) {
      ...
      for (const [key, state] of states) {
        if (!key.endsWith(suffix) || !state.componentRef) continue;
        refs.push(state.componentRef);
      }
      if (refs.length === 0) return;
      context.plugin.managers.structure.hierarchy.toggleVisibility(
        refs,
...
async function replaceComponent(
  ...
  if (channel === 'bonds') {
    // Always include `element-sphere` so bond ends are capped — without it
    // every cylinder is flat-cut and a bonds-only selection looks broken.
    // Atom-channel spacefill (when present) is larger and visually covers
    // these small caps, so combined `atoms` + `bonds` calls still look right.
    typeParams.visuals = ['element-sphere', 'intra-bond', 'inter-bond'];
  }
