...
test('search filters the table', async ({ page }) => {
  ...
  await page.getByPlaceholder(/Filter… \(e\.g\. nbResidues/).fill('lactamase');
  ...
  await expect(page.locator('.searchbox-count')).toHaveText('1 / 2');
});

test('search with no matches shows the empty placeholder', async ({ page }) => {
  await page.goto('/browse');
  await page.getByPlaceholder(/Filter… \(e\.g\. nbResidues/).fill('zzzz-no-match');
